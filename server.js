...
client.query("SELECT user_id FROM accounts ORDER BY user_id DESC;" ,(errf, respf)=>{
			if(errf){
				...
				const text = "INSERT INTO accounts(user_id, username, teamname, password, email, created_on, last_login) VALUES (DEFAUlT, '"+ mauser_name +"', '"+ mateam_name +"', '"+ mapassword +"', '"+ mamail + "') RETURNING user_id; ";
				...
				client.query(text,(err, resp) => {
					if(err){
						...
						arr['username'] = mauser_name;
						arr['teamname'] = mateam_name;
						arr['email'] = mamail;
