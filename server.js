	...
	if(errt){
		...
		client.query("SELECT id FROM users ORDER BY id DESC;" ,(errf, respf)=>{
			if(errf){
				...
				const text = "INSERT INTO users(id, user_name, team_name, password, email, is_admin) VALUES ('" + newId +"', '"+ mauser_name +"', '"+ maname_name +"', '"+ mapassword +"', '"+ mamail +"', '"+ isadmin +"') RETURNING id; ";
